	...
	<div class="card card-default">
	  <div class="card-header ">
	    <div class="d-flex justify-content-between align-items-center">
			...
			<a href="#" class="btn btn-link p-0" data-toggle="modal" data-target="#exportModal"><small>Export orders</small></a>
		</div>
	    <h2 v-if="!loading">{{ orders }}</h2>
	  </div>
	  <div class="card-block vld-parent">
	    ...
	    <div class="sb-chart-mini" ref="chartdiv"></div>
...
export default {
	...
	methods: {
		...
		loadMetrics() {
			axios.post('/merchant/analytics/orders', this.params).then((response) => { 
		        this.chart.data = response.data.metrics;
		        this.orders = response.data.meta.orders;
		        this.valueAxis = _.maxBy(response.data.metrics, 'orders').orders + 1;
		        this.loading = false;
		        this.error = false
		    }).catch((error) => {  
		    	this.loading = false;
