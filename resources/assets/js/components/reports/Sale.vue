...
export default {
	...
	methods: {
		...
		loadMetrics() {
			axios.post('/merchant/analytics/sales', this.params).then((response) => { 
		        this.chart.data = response.data.metrics;
		        this.sales = response.data.meta.sales;
		        this.valueAxis = _.maxBy(response.data.metrics, 'sales').sales + 1;
		        this.loading = false;
		        this.error = false
		    }).catch((error) => {  
		    	this.loading = false;
