            ...
            <div data-pages="card" class="card card-default" id="card-basic">
              <div class="card-header separator">
                <div class="card-title">
                  ...
                  <a v-if="response.allow.create" :href="addReturnEndpoint" class="btn btn-action-add btn-xs ml-2">instore return</a>
                </div>
              ...
              <div  class="card-block pt-20">
                  <div class="table-responsive" v-if="filteredRecords.length">
                      <table id="shippingClassTable" class="table table-hover tb100">
                          ...
                          <tbody>
                              <tr v-for="record in filteredRecords ">
                                ...
                                <td class="v-align-middle">{{ record.id }}</td>
                                <td class="v-align-middle"><a :href="record.order_url">{{ record.order }}</a></td>
                                <td class="v-align-middle">{{ record.date }}</td>
                                <td class="v-align-middle"><span class="label">{{ record.status }}</span></td>
                                <td class="v-align-middle" v-if="response.allow.deletion || response.allow.update">
                                  <div class="dropdown">
                                    <a :href="record.return_url">Edit</a>
                      ...
                      <pagination :pagination="response.pagination" for="returns" class="mt-4"></pagination>
                  </div>
                  <p class="text-center" v-else>No returns found.</p>
              </div>
            </div>
          </div>
        </div>
...
<script>
    ...
    export default {
        props: ['endpoint', 'addReturnEndpoint'],
        components: {
          Pagination
        },
        data () {
        ...
        methods: {
            ...
            destroy (selected) { 
                if(selected.length) {
                     this.deleteReturn(`${this.endpoint}/${selected}`)
                } else {
                    this.alert('Please choose at least one retuen to delete.')
                }
               
            },
            deleteReturn (endpoint) {
              if(this.response.allow.deletion) {
                swal({
                    title: 'Are you sure?',
                    text: "Deleted returns cannot be recovered. Do you still want to continue?",
                    type: 'warning',
                  ...
                  if (result.value) {
                    axios.delete(endpoint).then((response) => {
                        swal('Deleted!', 'Returns deleted successfully!', 'success');
        ...
        mounted() {
            ...
            eventHub.$on('returns.switched-page', this.getRecords)
