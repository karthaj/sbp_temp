            ...
            <div data-pages="card" class="card card-default" id="card-basic">
              <div class="card-header separator">
                <div class="card-title">
                  ...
                  <a v-if="response.allow.create" :href="shippingClassRoute" class="btn btn-action-add btn-xs">Add a Shipping Class</a>
                </div>
              ...
              <div  class="card-block pt-20">
                  <div class="table-responsive" v-if="filteredRecords.length">
                            ...
                            <template v-if="filteredRecords != 0">
                                <tr v-for="record in filteredRecords "> 
                                    ...
                                    <td class="v-align-middle" v-if="response.allow.deletion || response.allow.update">
                                      <div class="dropdown">
                                        <a v-if="response.allow.update" :href="'/merchant/store/shipping-class/'+record.slug+'/edit'">Edit</a>
                      ...
                      <pagination :pagination="response.pagination" for="shipping_classes" class="mt-4"></pagination>
                  </div>
                  <p class="text-center" v-else>No shipping classes found</p>
              </div>
            </div>
          </div>
        </div>
...
<script>
    ...
    export default {
        props: ['endpoint','shippingClassRoute'],
        components: {
          Pagination,
        },
        data () {
        ...
        methods: {
            ...
            destroy (selected) { 
                if(selected.length) {
                     this.deleteShippingClass(`${this.endpoint}/${selected}`)
                } else {
                    this.alert('Please choose at least one shipping class to delete.')
                }
               
            },
            deleteShippingClass (endpoint) {
              if(this.response.allow.deletion) {
                 swal({
                    title: 'Are you sure?',
                    text: "Hold on! Products which uses this shipping class will be assigned to default shipping class.",
                    type: 'warning',
        ...
        mounted() {
            this.getRecords()
            eventHub.$on('shipping_classes.switched-page', this.getRecords)
        },
    }
