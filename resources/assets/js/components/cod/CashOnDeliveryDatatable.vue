            ...
            <div data-pages="card" class="card card-default" id="card-basic">
              <div class="card-header separator">
                <div class="card-title">
                  ...
                  <a v-if="response.allow.create" :href="route" class="btn btn-action-add btn-xs">Add Cash On Delivery</a> 
                </div>
              ...
              <div  class="card-block pt-20">
                  <div class="table-responsive" v-if="filteredRecords.length">
                      <table id="shippingClassTable" class="table table-hover">
                          <thead>
                              <tr>
                                  ...
                                  <th class="sorting" @click="sortBy('zone_name')">
                                    zone name
                                    <i v-if="sort.key === 'zone_name'" :class="{ 'pg-arrow_up' : sort.order === 'asc', 'pg-arrow_down' : sort.order === 'desc' }"></i>
                                  </th>
                                  <th class="sorting" @click="sortBy('surcharge')">
                                    surcharge
                                    <i v-if="sort.key === 'surcharge'" :class="{ 'pg-arrow_up' : sort.order === 'asc', 'pg-arrow_down' : sort.order === 'desc' }"></i>
                                  </th>
                                  <th class="sorting" @click="sortBy('remark')">
                                    remark
                                    <i v-if="sort.key === 'remark'" :class="{ 'pg-arrow_up' : sort.order === 'asc', 'pg-arrow_down' : sort.order === 'desc' }"></i>
                                  </th>
                          ...
                          <tbody>
                            <template v-if="filteredRecords != 0">
                                <tr v-for="record in filteredRecords "> 
                                    ...
                                    <td class="v-align-middle">{{ record.surcharge }}</td>
                                    <td class="v-align-middle">{{ record.remark }}</td>
                                    ...
                                    <td class="v-align-middle" v-if="response.allow.deletion || response.allow.update">
                                      <div class="dropdown">
                                        <a v-if="response.allow.update" :href="'/merchant/store/payments/cashondelivery/'+record.id+'/edit'">Edit</a>
                      ...
                      <pagination :pagination="response.pagination" for="cod" class="mt-4"></pagination>
                  </div>
                  <p class="text-center" v-else>No cash on deliveries enabled.</p>
              </div>
            </div>
          </div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            destroy (selected) { 
                if(selected.length) {
                     this.deleteCOD(`${this.endpoint}/${selected}`)
                } else {
                    this.alert('Please choose at least one cash delivery method to delete..')
                }
                ...
                swal({
                  ...
                  if (result.value) {
                    axios.delete(endpoint).then((response) => {
                        swal('Deleted!', 'COD deleted successfully!', 'success');
        ...
        mounted() {
            ...
            eventHub.$on('cod.switched-page', this.getRecords)
        },
    }
