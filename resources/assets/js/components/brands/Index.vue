            ...
            <div data-pages="card" class="card card-default" id="card-basic">
              <div class="card-header separator">
                <div class="card-title">
                  ...
                  <a :href="route" class="btn btn-action-add btn-xs ml-2" v-if="response.allow.create">add brand</a>
                </div>
              ...
              <div  class="card-block pt-20">
                  <div class="table-responsive" v-if="filteredRecords.length">
                      <table id="brandTable" class="table table-hover">
                          <thead>
                              <tr>
                                  ...
                                  <th class="sorting" @click="sortBy('name')">products
                          ...
                          <tbody>
                            <template v-if="filteredRecords != 0">
                                <tr v-for="record in filteredRecords "> 
                                    ...
                                    <td class="v-align-middle">{{ record.products }}</td>
                                    <td class="v-align-middle" v-if="response.allow.deletion || response.allow.update">
                                      <div class="dropdown">
                                        <a v-if="response.allow.update" :href="record.edit">Edit</a>
                      ...
                      <pagination :pagination="response.pagination" for="brands" class="mt-4"></pagination>
                  </div>
...
<script>
    ...
    export default {
        props: ['endpoint', 'updateuri', 'route'],
        components: {
          Pagination,
        },
        data () {
        ...
        methods: {
            ...
            destroy (selected) { 
                if(selected.length) {
                     this.deleteBrand(`${this.endpoint}/${selected}`)
                } else {
                    this.alert('Please choose at least one brand to delete.')
                }
               
            },
            deleteBrand (endpoint) {
              if(this.response.allow.deletion) {
                swal({
                    title: 'Are you sure?',
                    text: "This will permanently remove the selected brands. Any products attached with this brand wil no longer be attached but products will be visible.",
                    type: 'warning',
                  ...
                  if (result.value) {
                    axios.delete(endpoint).then((response) => {
                        swal('Deleted!', 'Brands deleted successfully!', 'success');
        ...
        mounted() {
            ...
            eventHub.$on('brands.switched-page', this.getRecords)
        },
    }
