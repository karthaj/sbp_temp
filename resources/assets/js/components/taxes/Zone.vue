            ...
            <div data-pages="card" class="card card-default" id="card-basic">
              <div class="card-header separator">
                <div class="card-title">
                  ...
                  <a :href="taxZoneRoute" class="btn btn-action-add btn-xs">Add a Tax Zone</a>
                  <a :href="taxClassRoute" class="btn btn-action-add btn-xs">Add a Tax Class</a>
                  <a :href="taxRateRoute" class="btn btn-action-add btn-xs">Add a Tax Rate</a>
                </div>
              ...
              <div  class="card-block pt-20">
                  <div class="table-responsive" v-if="filteredRecords.length">
                      <table id="zoneTable" class="table table-hover">
                          <thead>
                              <tr>
                                  ...
                                  <th class="sorting" @click="sortBy('type')">
                                    zone type
                                    <i v-if="sort.key === 'name'" :class="{ 'pg-arrow_up' : sort.order === 'asc', 'pg-arrow_down' : sort.order === 'desc' }"></i>
                                  </th>
                          ...
                          <tbody>
                              <tr v-for="record in filteredRecords "> 
                                  ...
                                  <td class="v-align-middle" v-if="response.allow.deletion || response.allow.update">
                                    <div class="dropdown">
                                      <a v-if="response.allow.update" :href="'/merchant/store/tax/tax-zone/'+record.id">Edit</a>
                      ...
                      <pagination :pagination="response.pagination" for="tax_zones" class="mt-4"></pagination>
                  </div>
...
<script>
    ...
    export default {
        props: ['endpoint','taxZoneRoute','taxClassRoute', 'taxRateRoute'],
        components: {
          Pagination,
        },
        data () {
        ...
        mounted() {
            ...
            eventHub.$on('tax_zones.switched-page', this.getRecords)
        },
    }
