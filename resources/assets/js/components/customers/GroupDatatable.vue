            ...
            <div data-pages="card" class="card card-default" id="card-basic">
              <div class="card-header separator">
                <div class="card-title">
                  ...
                  <a v-if="response.allow.create" :href="route" class="btn btn-action-add btn-xs">add group</a>
                </div>
              ...
              <div  class="card-block pt-20">
                  <div class="table-responsive" v-if="filteredRecords.length">
                      <table id="shippingClassTable" class="table table-hover">
                          <thead>
                              <tr>
                                  ...
                                  <th class="sorting" @click="sortBy('discount')">
                                    discount
                                    <i v-if="sort.key === 'discount'" :class="{ 'pg-arrow_up' : sort.order === 'asc', 'pg-arrow_down' : sort.order === 'desc' }"></i>
                                  </th>
                          ...
                          <tbody>
                            <template v-if="filteredRecords != 0">
                                <tr v-for="record in filteredRecords "> 
                                    ...
                                    <td class="v-align-middle">{{ record.discount }}</td>
                                    <td class="v-align-middle" v-if="response.allow.deletion || response.allow.update">
                                      <div class="dropdown">
                                        <a v-if="response.allow.update" :href="'/merchant/customers/groups/'+record.id+'/edit'">Edit</a>
                      ...
                      <pagination :pagination="response.pagination" for="customer_groups" class="mt-4"></pagination>
                  </div>
                  <p class="text-center" v-else>No customer groups have been created. Please click the "Add group..." button to create one.</p>
              </div>
            </div>
          </div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            destroy (selected) { 
                if(selected.length) {
                     this.deleteGroup(`${this.endpoint}/${selected}`)
                } else {
                    this.alert('Please choose at least one group to delete..')
                }
            ...
            deleteGroup (endpoint) {
              if(this.response.allow.deletion) {
                swal({
                    title: 'Are you sure?',
                    text: "Any customers in the selected group(s) will no longer belong to those groups and as such won't see the discounted prices. Are you sure you want to do this?",
                    type: 'warning',
                  ...
                  if (result.value) {
                    axios.delete(endpoint).then((response) => {
                        swal('Deleted!', 'Groups deleted successfully!', 'success');
        ...
        mounted() {
            ...
            eventHub.$on('customer_groups.switched-page', this.getRecords)
        },
    }
