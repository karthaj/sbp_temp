            ...
            <div data-pages="card" class="card card-default" id="card-basic">
              <div class="card-header separator">
                <div class="card-title">
                  ...
                  <a v-if="response.allow.create" :href="route" class="btn btn-action-add btn-xs">create discount</a>
                </div>
              ...
              <div  class="card-block pt-20">
                  <div class="table-responsive" v-if="filteredRecords.length">
                          ...
                          <thead>
                              <tr>
                                  ...
                                  <th class="sorting" @click="sortBy('name')">name
                                    <i v-if="sort.key === 'id'" :class="{ 'pg-arrow_up' : sort.order === 'asc', 'pg-arrow_down' : sort.order === 'name' }"></i>
                                  </th>
                                  <th class="sorting" @click="sortBy('code')">code
                                    <i v-if="sort.key === 'code'" :class="{ 'pg-arrow_up' : sort.order === 'asc', 'pg-arrow_down' : sort.order === 'desc' }"></i>
                                  </th>
                                  <th class="sorting" @click="sortBy('active')">status
                                    <i v-if="sort.key === 'active'" :class="{ 'pg-arrow_up' : sort.order === 'asc', 'pg-arrow_down' : sort.order === 'desc' }"></i>
                                  </th>
                                  <th class="sorting" @click="sortBy('expires_at')">expiry date
                                    <i v-if="sort.key === 'expires_at '" :class="{ 'pg-arrow_up' : sort.order === 'asc', 'pg-arrow_down' : sort.order === 'desc' }"></i>
                                  </th>
                          ...
                          <tbody>
                            <template v-if="filteredRecords != 0">
                                <tr v-for="record in filteredRecords "> 
                                    ...
                                    <td class="v-align-middle">{{ record.code }}</td>
                                    ...
                                    <td class="v-align-middle">{{ record.expires_at }}</td>
                                    <td class="v-align-middle" v-if="response.allow.deletion || response.allow.update">
                                      <div class="dropdown">
                                        <a v-if="response.allow.update" :href="'/merchant/discounts/'+record.id+'/edit'">Edit</a>
                      ...
                      <pagination :pagination="response.pagination" for="discounts" class="mt-4"></pagination>
                  </div>
                  <p class="text-center" v-else>No discounts have been created.</p>
              </div>
            </div>
          </div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            destroy (selected) { 
                if(selected.length) {
                     this.deleteDiscount(`${this.endpoint}/${selected}`)
                } else {
                    this.alert('Please choose at least one discount to delete.')
                }
               
            },
            deleteDiscount (endpoint) {
              if(this.response.allow.deletion) {
                swal({
                    title: 'Are you sure?',
                    text: "Deleted discounts cannot be recovered. Do you still want to continue?",
                    type: 'warning',
                  ...
                  if (result.value) {
                    axios.delete(endpoint).then((response) => {
                        swal('Deleted!', 'Discounts deleted successfully!', 'success');
        ...
        mounted() {
            ...
            eventHub.$on('discounts.switched-page', this.getRecords)
        },
    }
