        ...
        <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12">
                <div class="category-cover img-full">
                     <img :src="getBrandImage" :alt="brand.name">
                </div>
            </div>    
        </div>
        <div class="row">
             <div class="col-lg-12 col-md-12 col-sm-12">
                <div v-if="!loading" class="shop-layout">
                    ...
                    <div class="row">
                        <div v-if="products.length" v-for="product in products" :key="product.id" class="col-lg-3 col-md-3 col-sm-3">
                ...
                <p v-if="!loading && products.length === 0" class="text-center my-5">No product found under this brand.</p>
            </div>     
        </div>
     </div>
</div>
...
<script>
	...
	export default {
		...
		computed: {
			getBrandImage() {
				if(this.brand.image.large) {
					return this.brand.image.large;
				}
		...
		methods: {
			getProducts (page) {
				axios.get(`/api/brands/${this.brand.handle}/products`, {
					params: {
						page,
						sort: this.sortBy
					}					
		...
		created () {
	      this.brand = JSON.parse(document.getElementById('brandJson').innerHTML).data;
	    },
		mounted() {
