    ...
    <div id="Slideshow-{{ $section_id }}" data-section-id="{{ $section_id }}" data-section-type="slideshow" data-speed="{{ $section['settings']['slide_show_speed'] }}" class="slider-area mb-60">
        <div class="hero-slider owl-carousel">
            @foreach($section['block_order'] as $block)
                ...
                <div class="single-slider-3" style="{{ 'background-image:url('.$image.')' }}">   
                    <div class="slider-progress"></div>
                    <div class="container">
                        <div class="hero-slider-content-3">
                            <h1 style="color: {{ $section['settings']['text_color'] }}">{{ $section['blocks'][$block]['settings']['title'] }}</h1>
                            <h2 style="color: {{ $section['settings']['text_color'] }}">{{ $section['blocks'][$block]['settings']['text_1'] }}</h2>
                            <h4 style="color: {{ $section['settings']['text_color'] }}">{{ $section['blocks'][$block]['settings']['text_2'] }}</h4>
