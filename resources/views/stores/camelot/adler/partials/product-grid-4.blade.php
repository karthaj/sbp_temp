...
if(count($product)) {
  if($product && array_has($product['images'], 1)) {
    $hoverImage = $product['images'][0]['medium'] . ', ' . $product['images'][1]['medium'];
  } else if($product && array_has($product['images'], 0)) {
    $hoverImage = $product['images'][0]['medium'] . ', ' . $product['images'][0]['medium'];
  } else if($product && array_has($product['cover_image'], 'medium')) {
        ...
        <div class="clearfix">
          <div class="b-product_grid_toggle float-left">
              @if($product['special_price'] > 0)
                <span class="b-price">
                  <del><span class="money">{{ $product['price_min'] }}</span></del> 
