<template>
	<div>
		<div class="b-product_attr_single" v-for="option, index in productOptions" :key="option.id">
		    <ul class="pl-0 list-unstyled clearfix d-flex align-items-center" v-if="option.type === '[CS]'">
		      <li><span class="b-product_attr_title pt-1">{{ option.name }}:</span></li>
		      	<li>
		      		<label v-for="value in option.values" :key="value.id" class="mb-0" :class="{'active': chosen[index] === value.id}">
