  ...
  <div class="row">
    @if(auth()->user()->addresses->count())
      @foreach(auth()->user()->addresses as $address)
       <div class="col-sm-12 col-md-6  col-lg-4 card-deck mb-4">
         <div class="card">
          <div class="card-header">{{ $address->alias }} @if($address->default) (Default) @endif</div>
           <div class="card-body">
          ...
          <div class="card">
           <div class="card-body">
              <div class="card-text text-center">
                <i class="ti-plus ti-3x align-middle" aria-hidden="true"></i>
...
<form action="{{ route('customer.address.store') }}" method="post">
  ...
  <div class="row">
    ...
    <div class="col-md-6 form-group {{ session()->has('country') ? session('country')->iso_code !== 'LK' ? 'd-block' : 'd-none' : 'd-block' }}">
    ...
    <div class="col-md-6 form-group {{ session()->has('country') ? session('country')->iso_code === 'LK' ? 'd-block' : 'd-none' : 'd-none' }}">
      <label for="account_address_city">City</label>
      <select class="form-control{{ $errors->has('City') ? ' is-invalid' : '' }}" id="account_address_city" name="city">
      </select>
      @if($errors->has('city'))
        <div class="invalid-feedback">{{ $errors->first('city') }}</div>
      @endif
  ...
  <div class="row justify-content-center mt-4">
    <div class="col-md-2">
      <button type="submit" class="btn_1 btn-block">Save</button>
    </div>
  </div>
</form>
