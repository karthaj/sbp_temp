...
<div  class="card card-transparent mb-0">
  <div class="card-header">
    <h1 class="section-title">Add a Shipping Class</h1>
  </div>
                  ...
                  <div class="card-block no-padding">
                    <div id="card-advance" class="card card-default mb-0">
                      <div class="card-block">
                        <form id="shippingClassForm" action="{{ route('shipping.classes.store') }}" method="post" autocomplete="off">
                        {{ csrf_field() }}
                          <div class="row">
                              <div class="col-sm-6 form-group">
                                  <label for="name">Class Name</label>
                                  <input type="text" id="name" class="form-control{{ $errors->has('name') ? ' error' : '' }}" name="name" value="{{ old('name') }}" required>
                                  @if($errors->has('name'))
