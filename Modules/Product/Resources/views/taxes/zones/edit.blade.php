...
<div  class="card card-transparent">
  <div class="card-header ">
    ...
    <h1 class="section-title">Edit Tax Zone: {{ $tax_zone->name }}</h1>
  </div>
  <div class="m-0 row card-block">
      ...
      <div class="col-lg-8 sm-no-padding">
        <form id="zoneForm" action="{{ route('tax.zone.update',$tax_zone->id) }}" method="post" autocomplete="off" class="sodirty">
        {{ csrf_field() }}
        {{ method_field('PATCH') }}
        <input type="hidden" name="tax_zone" value="{{ $tax_zone->id }}">
        <div class="row">
            <div class="col-lg-12">
               <div class="card card-transparent">
                      ...
                      <div class="card-block">
                          <div class="row">
                              <div class="col-sm-6 form-group{{ $errors->has('zone_name') ? ' has-danger' : '' }}">
                                  ...
                                  <input type="text" id="zone_name" class="form-control{{ $errors->has('zone_name') ? ' form-control-danger' : '' }}" name="zone_name" value="{{ $tax_zone->name }}" required>
                          ...
                          <div class="row">
                            <div class="col-sm-12 form-group">
                              ...
                              <div class="radio radio-success">
                                <input type="radio" value="country" name="zone_type" id="country_type" {{ $tax_zone->type == 'country' ? 'checked' : '' }}>
                                <label for="country_type" class="text-normal">This tax zone is based on one or more countries</label>
                                <input type="radio" value="state" name="zone_type" id="state" {{ $tax_zone->type == 'state' ? 'checked' : '' }}>
                                <label for="state" class="text-normal">This tax zone is based on one or more states</label>
                                <input type="radio" class="text-normal"  value="zip" name="zone_type" id="zip" {{ $tax_zone->type == 'zip' ? 'checked' : '' }}>
                                <label for="zip" class="text-normal">This tax zone is based on one or more postal or ZIP codes</label>
                              </div>
                            </div>
                          </div>
                          <div id="zoneCountry" class="row" style="display: {{ $tax_zone->type == 'country' ? 'block;' : 'none;' }}">
                            <div class="col-sm-12 form-group{{ $errors->has('zone_country') ? ' has-danger' : '' }}">
                              ...
                              @if($countries->count())
                                @foreach($countries as $country)
                                  <option value="{{ $country->id }}" {{ $tax_zone->taxRules()->where('country_id', $country->id)->count() ? 'selected' : '' }}>{{ $country->name }}</option>
                                @endforeach
                              @endif
                              </select>
                          ...
                          <div id="zipCountry" class="row" style="display: {{ $tax_zone->type == 'zip' ? 'block;' : 'none;' }}">
                          ...
                          <div id="stateCountry" class="row" style="display: {{ $tax_zone->type == 'state' ? 'block;' : 'none;' }}">
                            <div class="col-sm-12 form-group{{ $errors->has('state_country') ? ' has-danger' : '' }}">
                              ...
                              @if($country_states->count())
                                ...
                                <option value="{{ $country_state->id }}" {{ $tax_zone->taxRules()->where('country_id', $country_state->id)->count() ? 'selected' : '' }}>{{ $country_state->name }}</option>
                                @endforeach
                              @endif
                              </select>
                          ...
                          <div id="stateList" class="row" style="display: {{ $tax_zone->type == 'state' ? 'block;' : 'none;' }}">
                            <div class="col-sm-12 form-group{{ $errors->has('states') ? ' has-danger' : '' }}">
                              <label for="states">States</label>
                              <select id="states" class="full-width{{ $errors->has('states') ? ' form-control-danger' : '' }}" name="states[]" multiple>
                                  <optgroup label="{{ $tax_zone->taxRules->first()->country->name }}">
                                    @foreach($tax_zone->taxRules->first()->country->states as $state)
                                    <option value="{{ $tax_zone->taxRules->first()->country->id.'-'.$state->id }}">{{ $state->name }}</option>
                                    @endforeach
                                  </optgroup>
                              </select>
                          ...
                          <div id="zipCode" class="row" style="display: {{ $tax_zone->type == 'zip' ? 'block;' : 'none;' }}">
                            <div class="col-sm-12 form-group{{ $errors->has('zip_code') ? ' has-danger' : '' }}">
                              <label for="zip_code">Zip / Postal codes</label>
                              <input class="tagsinput{{ $errors->has('zip_code') ? ' form-control-danger' : '' }}" type="text" id="zip_code" name="zip_code" value="{{ $tax_zone->taxRules->first()->zip_codes }}">
                          ...
                          <div class="row">
                            <div class="col-sm-12 form-group{{ $errors->has('zone_status') ? ' has-danger' : '' }}">
                              <div class="checkbox check-success">
                                <input type="checkbox" value="1" id="zone_status" name="zone_status" {{ $tax_zone->status ? 'checked' : '' }} class="{{ $errors->has('zone_status') ? ' form-control-danger' : '' }}">
