...
<div  class="card card-transparent">
  <div class="card-header ">
    ...
    <h1 class="section-title">Edit Tax Rate: {{ $tax->name.' in '.$tax->zone->name }}</h1>
  </div>
  @if ($errors->any())
  ...
  <div class="m-0 row card-block">
      ...
      <div class="col-lg-8 sm-no-padding">
        <form id="taxRateForm" action="{{ route('tax.rates.update',$tax->id) }}" method="post" autocomplete="off" class="sodirty">
        {{ csrf_field() }}
        {{ method_field('PATCH') }}
        <div class="row">
            <div class="col-lg-12">
                      ...
                      <div class="card-block">
                          <div class="row">
                              <div class="col-sm-6 form-group">
                                ...
                                <select id="tax_zone" class="full-width{{ $errors->has('tax_zone') ? ' error' : '' }}" name="tax_zone" data-init-plugin="select2">
                                  <option value="{{ $tax->zone->id }}">{{ $tax->zone->name }}</option>
                                </select>
                              </div>
                              <div class="col-sm-6 form-group">
                                  <label for="tax_name">Tax Name</label>
                                  <span class="ml-2" data-placement="right" data-toggle="tooltip" data-original-title="A name that describes this tax rate (such as GST, PST, etc). This will be shown on the order-summary breakdown.">
                                    <i class="fa fa-question-circle"></i>
                                  </span>
                                  <input type="text" id="tax_name" class="form-control{{ $errors->has('tax_name') ? ' error' : '' }}" name="tax_name" value="{{ $tax->name ? $tax->name : old('tax_name') }}">
                          ...
                          <div class="row">
                            <div class="col-sm-12 form-group">
                              ...
                              @foreach($tax->rates as $rate)
                                <div class="row align-items-center">
                                  <div class="col-sm-2 pl-0 form-group">
                                    <input type="number" class="form-control{{ $errors->has('rates.'.$rate->id) ? ' error' : '' }}" name="rates[{{ $rate->id.','.$rate->tax_class_id }}]" value="{{ $rate->rate * 100 }}" min="0">
                                  </div>
                                  <div class="col-sm-10 form-group" class="form-control">
                                    % for products marked as  {{ $rate->taxClass->name }}
                                  </div>
                                </div>
                                @if($errors->has('rates.'.$rate->id))
                                  <label class="error">{{ $errors->first('rates.'.$rate->id) }}</label>
                                @endif
                              @endforeach
                              ...
                              @if($tax_classes->count())
                                ...
                                <div class="row align-items-center">
                                  <div class="col-sm-2 pl-0 form-group">
                                    <input type="number" class="form-control{{ $errors->has('rates.'.$class->id) ? ' error' : '' }}" name="rates[{{ $class->id }}]" value="{{ old('rates.'.$class->id) }}">
                                  </div>
                                  <div class="col-sm-10 form-group" class="form-control">
                          ...
                          <div class="row">
                            <div class="col-sm-4 form-group">
                              ...
                              <input type="number" class="form-control{{ $errors->has('calculation_order') ? ' error' : '' }}" name="calculation_order" id="calculation_order" value="{{ $tax->priority ? $tax->priority : 1 }}" min="0">
