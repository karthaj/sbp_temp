...
class StockTransferController extends Controller
{
    public function index(Request $request)
    {
        ...
        if($request->store) {
            $transfers = Transfer::where('stock_request', 0)->where('store_location_id', $request->store)->latest()->paginate($request->limit);
        } else {
            $transfers = Transfer::where('stock_request', 0)->latest()->paginate($request->limit); 
        }
