...
{ 
    ...
    { 
        ...
        try {
            ...
            $records = $this->builder->select('id', 'store_id', 'name', 'slug', 'small_default')->where('store_id', $request->tenant()->id)
                        ->with(['products', 'store'])
    ...
    {  
        ...
        foreach($ids as $id) {
            $brand = Brand::where('store_id', session('store')->id)->where('id', $id)->first();

            if($brand) {
                $brand->deleted_at = $brand->freshTimestamp();
                $brand->deleted_at_tz = $brand->freshTimestamp()->timezone(session('store')->setting->timezone->timezone);
                $brand->save();
