                  ...
                  <div class="card-block no-padding">
                    <div id="card-advance" class="card card-default">
                      ...
                      <div class="card-block">
                        <form action="{{ route('cod.update', $cod) }}" method="post" class="sodirty">
                          {{ csrf_field() }}
                          {{ method_field('PATCH') }}
                          <div class="form-group{{ $errors->has('shipping_zone') ? ' has-danger' : '' }}">
                            <label for="shipping_zone">shipping zone</label>
                            <input type="text" class="form-control" id="shipping_zone" name="shipping_zone" value="{{ $cod->zone->zone_name }}" disabled>
                          ...
                          <div class="form-group{{ $errors->has('surcharge') ? ' has-danger' : '' }}">
                            ...
                            <div class="input-group">
                              ...
                              <input type="text"  class="form-control autonumeric{{ $errors->has('surcharge') ? ' form-control-danger' : '' }}" name="surcharge" value="{{ old('surcharge', $cod->surcharge) }}" id="surcharge">
                            </div>
                          ...
                          <div class="form-group{{ $errors->has('payment_info') ? ' has-danger' : '' }}">
                            ...
                            <textarea name="payment_info" id="payment_info" cols="30" rows="4" class="form-control{{ $errors->has('payment_info') ? ' form-control-danger' : '' }}" required>{{ old('payment_info', $cod->remark) }}</textarea>
                          ...
                          <div class="form-group">
                            ...
                            <input type="checkbox" id="status" name="status" data-init-plugin="switchery" data-size="small" data-color="info" value="1" @if($cod->status) checked @endif>
                          </div>
